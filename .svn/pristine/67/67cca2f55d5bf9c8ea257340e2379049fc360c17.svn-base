    ...
    <div class="not-position_root" ref="myRoot">
        <el-form>
            ...
            <div class="not-position_header">
                ...
                <el-input v-if="isShowProfile === 2" v-model="changeData.keyword" size="small" @keyup.enter.native="searchCon()" placeholder="支持乙方名、录入人搜索" class="input-with-select" clearable>
                    <el-button slot="append" icon="el-icon-search" @click="searchCon()"></el-button>
                </el-input>

                <el-select v-model="pkCode" placeholder="请选择园区" @change="searchCon()" clearable class="select_park" filterable size="small">
                ...
                <!-- 批量抄表 -->
                <electricityMeterBatchRecord v-if="isShowProfile === 2" :ctList="multipleSelection" class="son_components"></electricityMeterBatchRecord>
                ...
                <!-- 切换 -->
                <div class="iconBox">
                    <div class="toggleTableEle toggleIcon" @click="toggleTable()">
                        <el-tooltip class="item" effect="dark" content="切换为表格模式" placement="top">
                            <i class="iconfont el-icon-diy-moshiqiehuan"></i>
                        </el-tooltip>
                    </div>
                    <div class="toggleProfileEle toggleIcon active" @click="toggleProfile()">
        ...
        <div v-if="isShowProfile === 1" class="profile-main">
            <h2 style="margin-bottom: 10px;">电表监控图
            ...
            <ul class="profile-view clearfix">
                <li class="profile-row" v-for="(item, index) in dataList" :key="index">
                    ...
                    <div class="row-item">
                        <div v-if="item.meterList.length === 0" class="item-column notBiao" style="width: 100%; background: #f78989;">
                            <el-button type="danger" size="mini" round v-if="item.eleMeterName == null" @click="addEleMeter(item)">
                                未添加或未启用
                            </el-button>
                        </div>
                        <div v-else-if="item.meterList.length < 3" class="item-column" v-for="(v, i) in item.meterList" :key="i" :style="{'width': (100/item.meterList.length) + '%'}">
                            <div v-if="!v.electricityRecordEntity.recordMonth" class="notChaoBiao" style="background: #728db0;" @mouseover.stop.prevent="showBtn(v)" @mouseleave.stop.prevent="hideBtn()">
                                <div class="biao-info1">
                                    <p>表名：<span>{{v.electricityRecordEntity.eleMeterName}}</span></p>
                                    <p>使用度数：<span>无</span></p>
                                    <p>本次抄表时间：<span>无</span></p>
                                    <p>是否生成账单：<span>无</span></p>
                                </div>
                                <div class="manual_mater_btn" v-show="isShowBtn == v.id">
                                    <electricityMeterBatchRecordProfile style="display: inline-block;" :ctObj="item" :meterObj="v" @queryContract="queryContract">
                                    </electricityMeterBatchRecordProfile>
                                </div>
                            </div>
                            <div v-if="v.electricityRecordEntity.recordMonth" style="background: #BFE06C;">
                                <div class="biao-info">
                                    <p>表名：<span>{{v.electricityRecordEntity.eleMeterName}}</span></p>
                                    <p v-if="v.electricityRecordEntity.electricityBillingType == '1'">使用度数(峰平谷)：<span>{{v.electricityRecordEntity.realDegree}} kWh</span></p>
                                    <p v-if="v.electricityRecordEntity.electricityBillingType == '2'">使用度数(均价)：<span>{{v.electricityRecordEntity.flatDegree}} kWh</span></p>
                                    <p>本次抄表时间：<span>{{v.electricityRecordEntity.currentMeterRecordDate | timeDate}}</span></p>
                                    <p>是否生成账单：<span v-text="v.electricityRecordEntity.hasBill === true ? '是' : '否'"></span></p>
                                </div>
                            </div>
                        </div>
                        <div v-else class="item-column" v-for="(v, i) in item.meterList" :key="i" :style="{'width': (100/item.meterList.length) + '%'}">
                            <div v-if="!v.electricityRecordEntity.recordMonth" class="notChaoBiao" style="background: #728db0;" @mouseover.stop.prevent="showBtn(v)" @mouseleave.stop.prevent="hideBtn()">
                                <div class="biao-info1">
                            ...
                            <div v-if="v.electricityRecordEntity.recordMonth" style="background: #BFE06C;text-align: left;">
                                <div class="biao-info">
                                    <el-popover placement="top" width="200" trigger="hover">
                                        <p>表名：<span style="text-align: left;">{{v.electricityRecordEntity.eleMeterName}}</span></p>
        ...
        <el-table v-if="isShowProfile === 2" ref="multipleTable" style="border-top: white;" :data="contractList" border :highlight-current-row="true" size="small" @selection-change="handleSelectionChange" :height="tableHeight">
            ...
            <el-table-column header-align="center" width="170" label="电表名">
                <template slot-scope="scope">
                    <span v-if="scope.row.eleMeterName != null">{{ scope.row.eleMeterName }}</span>
                    <el-button type="danger" size="mini" round v-if="scope.row.eleMeterName == null" @click="addEleMeter(scope.row)">未添加或未启用
                    </el-button>
                </template>
            </el-table-column>
            ...
            <el-table-column header-align="center" fixed="right" label="操作" width="130">
                <template slot-scope="scope">
                    <electricityMeterBatchRecord style="display: inline-block;" v-if="showManualMeterBtn && scope.row.eleMeterName != null" :ctObj="scope.row">
                    </electricityMeterBatchRecord>
                    <el-button v-show="showManualMeterBtn && scope.row.eleMeterName != null" style=" margin-left: 10px;" @click="addEleMeter(scope.row)" type="text" size="mini">
                        表管理
                    </el-button>

                    <el-button v-show="showManualMeterBtn && scope.row.eleMeterName == null" style="color: rgb(245, 108, 108);" @click="addEleMeter(scope.row)" type="text" size="mini">
                        添加电表
                    </el-button>
                </template>
            </el-table-column>
                ...
                <el-row>
                    <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
                        <el-form-item label="收款截止时间" prop="receivingDate">
                            <el-date-picker v-model="eleReceiptObj.receivingDate" type="date" placeholder="选择日期">
                            </el-date-picker>
                        </el-form-item>
                    </el-col>
                    <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
                        <el-form-item label="所属月份">
                            <el-date-picker v-model="eleReceiptObj.recordMonth" type="month" placeholder="选择日期">
                            </el-date-picker>
                        </el-form-item>
                    </el-col>
                </el-row>
...
<script>
    ...
    import electricityMeterBatchRecord from "../components/electricityMeterBatchRecord.vue";
    import electricityMeterBatchRecordProfile from "../components/electricityMeterBatchRecordProfile.vue";
    export default {
        components: {
            ElCol,
            ElRow,
        ...
        async created() {
            ...
            const res = await this.$ajax.post(`/parkButtJoint/query`,
                params
            );
        ...
        methods: {
            ...
            async queryContract() {
                ...
                if (this.isShowProfile === 1) {
                    ...
                    res = await this.$ajax.post(`contract/findEleMeterRecord`, param);
            ...
            submitBill(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        let params = {
                            entity: {
                                recordMonth: this.eleReceiptObj.recordMonth
                            },
                            receivingDate: this.eleReceiptObj.receivingDate,
                        ...
                        this.$ajax(
                            ...
                            if (response) {
                                ...
                                let data = {
                                    flag: "add",
                                    menuName: "电费应收报表",
                                    item: "electricityReceivableReport",
                                    params: {
                                        receivingDate: this.eleReceiptObj.receivingDate,
                                        pkCode: this.pkCode
                                    }
            ...
            toggleTable() {
                ...
                this.$nextTick(() => {
                    var toggleTableEle = document.querySelector(".toggleTableEle");
                    var toggleProfileEle = document.querySelector(".toggleProfileEle");
                    toggleTableEle.classList.add("active");
                    toggleProfileEle.classList.remove("active");
                    this.searchCon();
                })
            },
            // 切换刨面图
            toggleProfile() {
                ...
                this.$nextTick(() => {
                    var toggleTableEle = document.querySelector(".toggleTableEle");
                    var toggleProfileEle = document.querySelector(".toggleProfileEle");
                    toggleProfileEle.classList.add("active");
                    toggleTableEle.classList.remove("active");
                    this.searchCon();
                })
            },
        }
