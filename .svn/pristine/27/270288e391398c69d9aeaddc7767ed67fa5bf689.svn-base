...
import com.kfwy.park.controller.report.vo.WaterReceivableBiServiceRequest;
 ...
 */
@Component
@RequestMapping("/waterReceivable")
public class WaterReceivableBiService extends SpringRestService {
    ...
    public ResponseEntity<IServiceResponse> query(@RequestBody WaterReceivableBiServiceRequest request) {

        AccountReceivableBiServiceResponse response = new AccountReceivableBiServiceResponse();
        WaterReceivableBiEntity waterReceivableBiEntity = request.getEntity();

        Map<String, Object> map = new HashMap<>();

        if (waterReceivableBiEntity.getStartDate() != null && waterReceivableBiEntity.getEndDate() != null) {
            Date dayBegin = DateFormatUtil.dayBegin(waterReceivableBiEntity.getStartDate());
            Date dayEnd = DateFormatUtil.dayEnd(waterReceivableBiEntity.getEndDate());
        ...
        List<WaterReceivableBiEntity> waterReceivableList = waterReceivableBiManager.selectWaterReceivableReport(map);

        response.setResult(waterReceivableList);
     ...
     * @return void
     **/
    @RequestMapping(path = "/exportWaterReceivableBiExcel",
    ...
    public void export(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws FileExportException, IOException {
        ...
        String path = "/exportTemplate/" + "waterReceivableBiReport" + ".xml";
        ...
        List<WaterReceivableBiEntity> data = waterReceivableBiManager.selectWaterReceivableReport(map);
        if(data.size() <= 0) {
            throw new BusinessException("无数据导出");
        }
        ExportFileUtil exportFileUtil = new ExportFileUtil();
        exportFileUtil.exportExcel(httpServletResponse, WaterReceivableBiService.class, path, data, name.toString() + "的应收水费报表.xlsx");
    }
}
