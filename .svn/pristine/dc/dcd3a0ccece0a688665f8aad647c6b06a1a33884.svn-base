<template>
    <div>
        <el-card shadow="hover">
            <div slot="header" style="height: 30px; line-height: 30px; text-align: left;">
                <el-row :gutter="10">
                    <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
                        <div class="park-data-title-font">应收电费用异常看板：{{pkName}}</div>
                    </el-col>
                    <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
            ...
            <el-table :data="eleExcObj.eleExceptionTable"
                      max-height="400"
                      border>
                <el-table-column
                        prop="companyName"
...
<script scoped>
    export default {
        ...
        created: function () {
            ...
            if (this.param) {
                this.eleExcObj.pkCode = this.param;
                //查询园区名
                this.queryParkName(this.eleExcObj.pkCode);
            } else {
                this.eleExcObj.pkCode = "";
                this.pkName = "全部园区";
            }
        ...
        methods: {
            ...
            queryParkName(pkCode){
                ...
                this.$ajax(
                    ...
                    vm.$alert('页面:电费应收异常看板查询园区异常', '系统异常', {
            ...
            queryList() {
                ...
                parkParam.entity = {};
                parkParam.entity.pkCode = this.eleExcObj.pkCode;

                let option = {
                    method: 'POST',
                    data: parkParam,
                    url: 'contract/queryEleCostException'
                };
                this.$ajax(
                    option
                ).then(function (response) {
                    vm.total = response.data.result.totalCount;
                    let costExcList = response.data.result.data;
                    if (costExcList && costExcList.length > 0) {
                        let index = 0;
                        let companyName, customerTypeName, relateName, relatePhone;

                        vm.eleExcObj.pkName = costExcList[0].pkName;
                        let arr = [];
                        for (let i = 0; i < costExcList.length; i++) {
                            ...
                            for (let j = 0; j < costExcList[i].electricityReceiptList.length; j++) {
                                let paramList = {};
                                ...
                                paramList.lastBillingTime = costExcList[i].electricityReceiptList[j].lastBillingTime;
                                paramList.currentBillingTime = costExcList[i].electricityReceiptList[j].currentBillingTime;
                                paramList.receivablePrice = costExcList[i].electricityReceiptList[j].receivablePrice;
                                paramList.realPrice = costExcList[i].electricityReceiptList[j].realPrice;

                                arr[index] = paramList;
                                index = index + 1;
                            }
                        }
                        vm.eleExcObj.eleExceptionTable = arr;
                    } else {
                        vm.eleExcObj.eleExceptionTable = [];
                    }
                }).catch(function (error) {
                    vm.$alert('页面:查询电费异常看板信息失败', '系统异常', {
                        confirmButtonText: '确定',
                        callback: action => {
                        }
                    });
            ...
            //配置按钮
            editBoard() {
                this.dialogTitle = "应收电费费用异常看板参数配置";
                let vm = this;
                ...
                vm.parkParam.pkCode = vm.eleExcObj.pkCode;
