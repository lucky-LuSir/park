            ...
            <div slot="header" style="height: 30px; line-height: 30px; text-align: left;">
                <el-row :gutter="10">
                    <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
                        <div class="park-data-title-font">应收水费用异常看板：{{pkName}}</div>
                    </el-col>
                    <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
                        <el-button class="edit-button" size="small" type="primary" @click="deleteBoard">删除</el-button>
                        <el-button class="edit-button" size="small" type="primary" @click="editBoard">配置</el-button>
                    </el-col>
                </el-row>
            </div>

            <el-table :data="waterExcObj.waterExceptionTable"
...
<script scoped>
    export default {
        ...
        created:function () {
            ...
            if (this.param) {
                this.waterExcObj.pkCode = this.param;
                //查询园区名
                this.queryParkName(this.waterExcObj.pkCode);
            } else {
                this.waterExcObj.pkCode = "";
                this.pkName = "全部园区";
            }
            this.queryList();
        },
        ...
        methods: {
            ...
            queryParkName(pkCode){
                ...
                this.$ajax(
                    ...
                    vm.$alert('页面:应收水费用异常看板查询园区信息失败', '系统异常', {
            ...
            queryList() {
                ...
                parkParam.entity = {};
                parkParam.entity.pkCode = this.waterExcObj.pkCode;

                let option = {
                    method: 'POST',
                    data: parkParam,
                    url: 'contract/queryWaterCostException'
                };
                this.$ajax(
                    option
                ).then(function (response) {
                    ...
                    if (costExcList && costExcList.length > 0){
                        ...
                        vm.waterExcObj.pkName = costExcList[0].pkName;
                        let arr=[];
                        for (let i=0;i<costExcList.length;i++) {
                            ...
                            for (let j=0;j<costExcList[i].waterReceiptList.length;j++) {
                                let paramList = {};
                                ...
                                paramList.lastBillingTime = costExcList[i].waterReceiptList[j].lastBillingTime;
                                paramList.currentBillingTime = costExcList[i].waterReceiptList[j].currentBillingTime;
                                paramList.receivablePrice = costExcList[i].waterReceiptList[j].receivablePrice;
                                paramList.realPrice = costExcList[i].waterReceiptList[j].realPrice;

                                arr[index]= paramList;
                                index = index + 1;
                            }
                        }
                        vm.waterExcObj.waterExceptionTable = arr;
                    } else {
                        vm.waterExcObj.waterExceptionTable = [];
                    }
                }).catch(function (error) {
                    vm.$alert('页面:查询应收水费用异常看板信息失败', '系统异常', {
                        confirmButtonText: '确定',
                        callback: action => {
                        }
                    });
...
//                            this.$router.replace('/back');
                        ...
                        vm.$alert('页面:删除应收水费用异常看板失败', '系统异常', {
                            confirmButtonText: '确定',
                            callback: action => {
                            }
                        });
            ...
            //配置按钮
            editBoard() {
                this.dialogTitle = "应收水费费用异常看板参数配置";
                let vm = this;
                ...
                this.$ajax(
                    ...
                    vm.$alert('页面:应收水费用异常看板查询房源信息失败', '系统异常', {
                        confirmButtonText: '确定',
                        callback: action => {
                        }
                    });
                });
                vm.parkParam.pkCode = vm.waterExcObj.pkCode;
            },
                ...
                this.$refs[subForm].validate((valid) => {
                    if (valid) {
                        ...
                        this.$ajax(
                            ...
                            vm.$alert('页面:配置应收水费用异常看板失败', '系统异常', {
                                confirmButtonText: '确定',
                                callback: action => {
                                }
                            });
