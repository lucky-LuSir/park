<template>
    <div>
        <el-card shadow="hover">
            <div slot="header" style="height: 30px; line-height: 30px; text-align: left;">
                <el-row :gutter="10">
                    <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
                        <span class="park-data-title-font">应收租金费用异常看板：{{ pkName }}</span>
                    </el-col>
                    <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
            ...
            <el-table :data="rentExcObj.rentExceptionTable"
                      max-height="400"
                      border>
                <el-table-column
                        prop="companyName"
        ...
        <!--<div class="comm-title-div">-->
            <!--<div class="park-data-title-font">应收租金费用异常看板：{{pkName}}</div>-->
            <!--<el-button class="edit-button" size="small" type="primary" @click="deleteBoard">删除</el-button>-->
            <!--<el-button class="edit-button" size="small" type="primary" @click="editBoard">配置</el-button>-->
        <!--</div>-->
...
<script scoped>
    export default {
        ...
        created:function () {
            ...
            if (this.param) {
                this.rentExcObj.pkCode = this.param;
                //查询园区名
                this.queryParkName(this.rentExcObj.pkCode);
            } else {
                this.rentExcObj.pkCode = "";
                this.pkName = "全部园区";
            }
            //解析参数param,调相应方法
            this.queryList();
        ...
        methods: {
            ...
            queryParkName(pkCode){
                ...
                this.$ajax(
                    ...
                    vm.$alert('页面:应收租金费用异常看板查询园区信息失败', '系统异常', {
            ...
            queryList() {
                ...
                parkParam.entity = {};
                parkParam.entity.pkCode = this.rentExcObj.pkCode;

                let option = {
                    method: 'POST',
                    data: parkParam,
                    url: 'contract/queryRentCostException'
                };
                this.$ajax(
                    option
                ).then(function (response) {
                    ...
                    if (costExcList && costExcList.length > 0){
                        ...
                        vm.rentExcObj.pkName = costExcList[0].pkName;
                        let arr=[];
                        for (let i=0;i<costExcList.length;i++) {
                            ...
                            for (let j=0;j<costExcList[i].rentReceiptList.length;j++) {
                                let paramList = {};
                                ...
                                paramList.rentStartTime = costExcList[i].rentReceiptList[j].rentStartTime;
                                paramList.rentEndTime = costExcList[i].rentReceiptList[j].rentEndTime;
                                paramList.receivableRent = costExcList[i].rentReceiptList[j].receivableRent;
                                paramList.realRent = costExcList[i].rentReceiptList[j].realRent;

                                arr[index]= paramList;
                                index = index + 1;
                            }
                        }
                        vm.rentExcObj.rentExceptionTable = arr;
                    } else {
                        vm.rentExcObj.rentExceptionTable = [];
                    }
                }).catch(function (error) {
                    vm.$alert('页面:查询应收租金费用异常信息失败', '系统异常', {
                        confirmButtonText: '确定',
                        callback: action => {
                        }
                    });
...
//                            this.$router.replace('/back');
                        ...
                        vm.$alert('页面:删除应收租金费用异常看板失败', '系统异常', {
                            confirmButtonText: '确定',
                            callback: action => {
                            }
                        });
            ...
            editBoard() {
                ...
                vm.parkParam.pkCode = vm.rentExcObj.pkCode;
...
//                            vm.$router.replace('/back');
                            ...
                            vm.$alert('页面:配置应收租金费用异常看板失败', '系统异常', {
                                confirmButtonText: '确定',
                                callback: action => {
                                }
                            });
