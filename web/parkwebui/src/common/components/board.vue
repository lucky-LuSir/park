<template>
    <div>
        <el-button type="primary" size="small" @click="isShowBoard = true" style="margin-left: 10px;">添加看板</el-button>
        <el-dialog title="添加看板" :visible.sync="isShowBoard" width="60%" center top="5vh" :before-close="handleClose" :append-to-body='true' style="height: 780px">
            <div class="left">
            ...
            <div class="right">
                <div style="height: 100%;overflow:auto;">
                    <div v-for="(item, index) in programs" :key="index" style="height: 75px;padding: 10px;border-bottom: 1px solid #ccc;">
                        <div style="width: 25%;float: left; height: 100%;">
                            <img src="../assets/img/thumb.png" style="width: 90%;height: 100%"/>
                        </div>
                        <div style="width: 49%;float: left;height: 100%;">
                            <h4 class="addBoard_title">{{ item.pgName }}</h4>
                            <p>{{ item.remark }}</p>
                        </div>
                        <div style="width: 25%;float: right; height: 100%;">
                            <el-button type="primary" @click="isShowParam(item.pgCode)">添加看板</el-button>
                        </div>
        ...
        <!--mine参数弹框-->
        <el-dialog  :title="dialogTitle" :visible.sync="isShowMineParam"  :append-to-body='true'>
        ...
        <!--园区参数弹框-->
        <el-dialog  :title="dialogTitle" :visible.sync="isShowParkParam"  :append-to-body='true'>
        ...
        <!--部门参数弹框-->
        <el-dialog  :title="dialogTitle" :visible.sync="isShowDeptParam"  :append-to-body='true'>
...
//                    ],
        ...
        methods: {
            ...
            isShowParam(pgCode){
                ...
                if (pgCode === "pg009") {
                    this.isShowMineParam = true;
                    this.dialogTitle = "应收情况总览";
                    this.onPgCode = "009";
                }
                if (pgCode === "pg010") {
                    this.isShowMineParam = true;
                    this.dialogTitle = "毛利率统计";
                    this.onPgCode = "010";
                }

                if (pgCode === "pg011") {
                    this.isShowMineParam = true;
                    this.dialogTitle = "去划面积统计看板";
                    this.onPgCode = "011";
                }

                if (pgCode === "pg012") {
                    this.isShowMineParam = true;
                    this.dialogTitle = "数据统计看板";
                    this.onPgCode = "012";
                }
            ...
            searchBoard() {
                ...
                let option = {
                    method: 'POST',
                    data: obj,
                    url: 'program/selectBoardByKeyWord'
                };
                this.$ajax(
                    option
                ).then(function (response) {
            ...
            onParkParamSubmit(subForm) {
                this.$refs[subForm].validate((valid) => {
                    if (valid){
                        ...
                        if(this.onPgCode == "002") {
                            if(this.parkParam.pkCode == "") {
                                this.$alert('园区去化看板：请指定某一个园区查看去划看板', '温馨提示', {
                                    confirmButtonText: '确定',
                                    callback: action => {
                                    }
                                });
